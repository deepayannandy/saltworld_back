...
router.post("/:clientId", verifyToken, async (req, res) => {
  ...
  const { value } = clientAppointmentCreateValidator(req.data);

  const startDateTime = new Date(value.startDateTime);
  const endDateTime = addMinutes(startDateTime, req.body.Duration);
  ...
  try {
    client.appointments.push({ ...value, endDateTime });
    client = await client.save();
    const latestClientAppointment = client.appointments.find(
      (appointment) => appointment.title === value.title
    );
...
router.get("/:clientId", verifyToken, async (req, res) => {
  ...
  const appointment = client.appointments.find((appointment) => appointment._id === req.body.id);
...
router.get("/client/:clientId", verifyToken, async (req, res) => {
  ...
  try {
    const appointmentsData = client.appointments.map((appointment) => {
      ...
      return Object.assign({}, schedule.toObject(), {
        date,
        startTime,
        endTime,
        status,
...
//get all appointments
router.get("/existing/:time&:serviceId&:duration", verifyToken, async (req, res) => {
  if (req.tokendata.UserType !== "Admin") {
  ...
  try {
    ...
    for (const client of clients) {
      const appointments = client.appointments.filter((appointment) => {
        ...
        if (appointment.startDateTime >= startDateTime && appointment.startDateTime <= endDateTime && appointment.serviceId === req.params.serviceId) {
          return true;
        }
        return false;
      })
...
router.get("/searchbydate/:starttime&:endtime&:branch", verifyToken, async (req, res) => {
  if (req.tokendata.UserType !== "Admin") {
    return res.status(400).json({ message: "Access Prohibited!" });
  }
  ...
  try {
    ...
    for (const client of clients) {
      const appointments = client.appointments.filter((appointment) => {
        ...
        if (appointment.startDateTime >= startDateTime && appointment.startDateTime <= endDateTime && appointment.branch === req.params.branch) {
          return true;
        }
        return false;
      })
...
router.patch("/:id", verifyToken, async (req, res) => {
  ...
  const appointment = client.appointments.find((appointment) => appointment._id === req.params.id);
  if (!appointment) {
