...
router.get("/client/:clientId", async (req, res) => {
  try {
    ...
    const scheduleData = schedules.map((schedule) => {
      const date = format(schedule.startdatetime, "dd-MMM-yyyy");
      ...
      const status =
        new Date() < schedule.startdatetime
          ? "Upcoming"
          : new Date() > schedule.enddatetime ? 'Completed' : 'Live';
...
router.patch("/:id", getSchedule, async (req, res) => {
  ...
  try {
    ...
    res.schedule.title =
      res.schedule.servicedetails +
      " for " +
      res.schedule.clientname +
      "(" +
      res.schedule.personcount +
      ") at " +
      res.schedule.startdatetime;
...
router.delete("/:id", getSchedule, async (req, res) => {
  try {
    ...
    const deldata = await ScheduleModel.deleteOne({
